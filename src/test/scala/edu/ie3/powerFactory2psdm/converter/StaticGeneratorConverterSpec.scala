...
class StaticGeneratorConverterSpec extends Matchers with AnyWordSpecLike {
  "A static generator converter" should {
    "convert a list of static generators correctly" in {
      ...
      val inputWecFixedFeedIn =
        statGen.copy(id = "WecFixedFeedIn", category = WEC)
      val individualWecConfig = IndividualWecConfig(
        Set("WecFixedFeedIn"),
        WecFixedFeedIn(FixedQCharacteristic)
      )
      val wecConversionConfig = statGenModelConfigs.wecConfig
        .copy(individualConfigs = Some(List(individualWecConfig)))

      val inputPvModel = statGen.copy(id = "PvModel", category = PV)
      val inputPvFixedFeedIn = statGen.copy(id = "PvFixedFeedIn", category = PV)
      val individualPvConfig = IndividualPvConfig(
        Set("PvFixedFeedIn"),
        PvFixedFeedIn(FixedQCharacteristic)
      )
      val pvConversionConfig = statGenModelConfigs.pvConfig
        .copy(individualConfigs = Some(List(individualPvConfig)))
      ...
      val actual = StaticGeneratorConverter.convert(
        inputs,
        statGenModelConfigs
          .copy(pvConfig = pvConversionConfig, wecConfig = wecConversionConfig),
        nodesMap
      )
